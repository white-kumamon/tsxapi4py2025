...
class DomSurfaceCoordinator:
    ...
    def _build_candles_locked(self) -> List[Dict[str, Any]]:
        """Aggregate trades into rolling 3-minute candlesticks."""

        trades = [trade for trade in self._trades if trade.get("timestamp")]
        ...
        for trade in sorted(trades, key=lambda item: item["timestamp"]):
            ...
            volume = _to_float(trade.get("volume")) or 0.0

            if timestamp is None or price is None:
                continue

            floored = timestamp - timedelta(
                minutes=timestamp.minute % 3,
                seconds=timestamp.second,
                microseconds=timestamp.microsecond,
            )

            bucket = buckets.get(floored)
            if bucket is None:
                bucket = {
                    "start": floored,
                    "end": floored + timedelta(minutes=3),
                    "open": price,
                    "high": price,
                    "low": price,
                    "close": price,
                ...
                buckets[floored] = bucket
                ordered.append(floored)
                continue
    ...
    def _build_snapshot_locked(self) -> Dict[str, Any]:
        ...
        snapshot = {
            ...
            "candles": self._build_candles_locked(),
            "order_book": {
                "bids": bids_payload,
                "asks": asks_payload,
            },
